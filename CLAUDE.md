...
- **Image-to-video fallback**: In max-extend mode, if the permalink has no video (e.g. static image), this is detected once before the item loop via `_navigateToSourceVideo()`. The worker then uses the normal generation path (`_ensureOnPermalink` + `generator.generate()`) for all items, so content moderation retries stay on the same page. After each successful generation, the extend loop runs normally. `extendFromTime` is skipped for these chains (the generated video is too short). The `isImagePost` flag is set once; `generatedFromImage` passes through to `_runExtendLoop` via `options.skipExtendFromTime`.
