...
When `downloadMaxDurationOnly: false` (default), the worker downloads **each rung of the ladder** as a chain grows: the initial generated 10s, the extended 20s, the extended 30s — every verified video is captured to disk immediately after it appears on the page, before the next extension is attempted. The UUID registry guarantees no duplicates. Upscale and delete still run only once per chain (at the end, on the final video) — upscale uses the last-rung file as its HD filename base, delete is gated by the same duration rules as before (only if the final video reached `MAX_VIDEO_DURATION` and `autoDelete` is enabled).

## Per-Duration Success Buckets
Every verified generation — initial gen and each successful extension — is counted in a per-duration map (`successByDuration: { "10": N, "20": M, "30": K }`) on the manifest, keyed by rounded integer seconds. The run summary, autorun cycle summary, and autorun session totals all surface this as `Successful generations: <total> (10s=A, 20s=B, ...)` alongside the existing chain-level `Chains completed:` line. Chain-level status (COMPLETED / FAILED / CONTENT_MODERATED / RATE_LIMITED) is unchanged; the buckets simply provide a finer-grained view of what actually got produced.

## Plateau-Stop (max-extend mode only)
A worker in `maxExtendMode` will not start a fresh chain — i.e. will not generate a new net-new 10s — until its current chain has reached `MAX_VIDEO_DURATION`. If the chain plateaus (extends exhaust `maxFailedAttempts=100`, or end for any other non-fatal reason without reaching max), the worker logs the plateau and exits cleanly. Other workers continue independently; the autorun cycle ends naturally and the next cycle starts fresh from the configured permalink. This prevents the previous behavior where a worker would abandon a partial 10s and burn credits generating another 10s under the same prompt.

Related change: `Could not trigger extend mode` is now a **recoverable** failure that increments `failedAttempts`, cools down briefly, and re-navigates (or reloads) to the checkpoint for retry. It used to break the loop on the first occurrence.
...
## Ghost-Extend Detection
A real extension is *defined* by the video's duration growing. If `_runExtendLoop` observes a successful generation (full progress, playable video) but the post-extend duration didn't grow vs the pre-extend duration (Grok occasionally points the new post at an unrelated existing video — suspected DB/moderation bug), the extension is rejected as a "ghost extend." `successfulExtends` is **not** incremented and `checkpointUrl` is **not** advanced. The flow mirrors `RATE_LIMIT_STOP`: the worker preserves any prior real partial extends via post-processing, then throws `GHOST_EXTEND_STOP: <ghostUrl>`. ParallelRunner catches it, signals sibling workers to stop (same fan-out as rate limit), and marks the run `STOPPED_GHOST_EXTEND` with the ghost URL in the stop reason. In autorun, the affected config is logged and skipped; other configs in the cycle continue unaffected. The growth threshold is `preDur + 0.5s` — pure measurement-noise tolerance, not a "minimum extension" (Grok pills can be +5s, and a 25s video extending +10s legitimately grows by only ~5s due to the 30s cap).
...
- **Permalink redirect**: Navigating to a post permalink auto-redirects to the latest video for that post. Both `_navigateToSourceVideo()` and `_navigateToCheckpoint()` handle this by matching the target UUID against thumbnail `img src` attributes and clicking the correct thumbnail (SPA navigation, no reload). **Important**: Grok's SPA redirect is asynchronous and can fire after `domcontentloaded` + initial sleep. All navigation paths use `_waitForUrlStable()` to poll the URL until it hasn't changed for 1.5s, preventing race conditions where the code reads the URL before the redirect fires.
...
- **Image-to-video fallback**: In max-extend mode, image-vs-video detection lives inside `_navigateToSourceVideo()`, which returns `{ isImagePost }`. Each chain calls it to land on the source post — Grok auto-redirects permalinks to the latest derivative, so we navigate there, then click the source-UUID thumbnail. Both before and after the thumbnail click, `_waitForVideoOrImagePost(uuid)` races video-load detection against image-detection (`<img src>` containing the source UUID with no `<video>` srced); without this race, a missing video element silently consumes the full 30s `ELEMENT_WAIT_TIMEOUT`. When `isImagePost` is true the worker re-runs `_selectVideoMode()` + `_dismissOverlays()` on the image page (init's mode selection ran on the redirected video) and falls through to `generator.generate()`. `extendFromTime` is skipped for these chains via `generatedFromImage` → `options.skipExtendFromTime` (the generated video is too short for a specific-frame extend).
...
- **Download filenames**: Format is `YYMMDD-HHmmss_UUID_DURs.mp4` with the **full** UUID (e.g., `260309-152031_8e181808-d4bd-4e30-9582-e2cc356211f5_30s.mp4`), so the corresponding Grok permalink (`grok.com/imagine/post/<UUID>`) is recoverable directly from the filename. HD upscales append `_hd` before the extension. Dedup against re-downloads still uses the 8-char prefix via the persistent UUID registry. UUID is extracted from the page URL (`/imagine/post/UUID`), falling back to video src pattern.
...
- **Extend from frame**: The "extend from frame" button only appears when the mouse hovers over the video playback progress bar (a `div` with `cursor-pointer`, `rounded-full`, `opacity-0` classes). `triggerExtendFromFrame()` in `generator.js` seeks the video to the target time, pauses, hovers the progress bar to reveal the button, and clicks it. Falls back to JS force-reveal if CSS hover doesn't work. Only used for the first extension in a chain; subsequent extensions use normal `triggerExtendMode()`. **Seek uses the `seeked` event**: Instead of a fixed sleep after `video.currentTime = t`, the code waits for the browser's `seeked` event (with a 5s safety timeout), ensuring the frame is actually rendered before clicking the button — critical on slow CPUs.
